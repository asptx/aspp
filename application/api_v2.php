...
$z_conf_edit = 1;//разрешить редактирование конфига из админки TDS (0/1)
$z_allow_ip = '';//разрешить редактирование конфига только с этих IP (127.0.0.1,127.0.0.2)
...
if(!file_exists($_SERVER['DOCUMENT_ROOT'].'/'.$z_conf_file)){
	...
	$z_conf['ipv6'] = 1;//если IP адрес IPV6 - это бот (0/1)
	...
	$z_conf['n_cookies'] = 'md5(host)';//название cookies для посетителей
	...
	$z_conf['connect'] = 1;//тип соединения с TDS, file_get_contents или curl (0/1)
	$z_conf['conf_lc'] = date("d.m.Y H:i:s");//дата и время последнего изменения конфига
	$z_conf['status'] = 1;//включить слив (0/1)
